...
async function renderClasses() {
  try {
    ...
    classes.forEach((classroom) => {
      ...
      details.textContent = `${classroom.subject} | Class code: ${classroom.code}`;
      const members = document.createElement("p");
      members.className = "text-slate-400";
      members.textContent = classroom.students.length
        ? `Students: ${classroom.students.map((item) => item.name).join(", ")}`
        : "No students enrolled yet.";
      const assignments = document.createElement("p");
      assignments.className = "text-slate-400";
      assignments.textContent = classroom.exams.length
        ? `Exams: ${classroom.exams.map((item) => item.title).join(", ")}`
        : "No exams assigned yet.";
      content.append(title, details, members, assignments);
      const remove = document.createElement("button");
      remove.className = "eg-btn-danger";
      remove.type = "button";
      remove.textContent = "Delete";
      remove.addEventListener("click", async () => {
        if (confirm("Delete this class and its enrollments?")) {
          await ExamGuardApi.deleteClass(classroom.id);
          renderClasses();
        }
      });
    ...
    classList.replaceChildren(emptyMessage(error.message));
  }
}
...
document.getElementById("createClassForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await renderClasses();
    alert(`Class created. Join code: ${classroom.code}`);
  } catch (error) {
    alert(error.message);
  }
});

document.getElementById("assignExamForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  if (!examSelect.value || !classSelect.value) return;
  try {
    await ExamGuardApi.assignExam(Number(examSelect.value), Number(classSelect.value));
    await renderClasses();
